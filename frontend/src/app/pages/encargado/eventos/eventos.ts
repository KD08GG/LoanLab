...
import {
  ...
  Users, Monitor, Laptop, Tablet, Cable, Pencil, Trash2
} from 'lucide-angular';
...
@Component({
  selector: 'app-eventos-encargado',
  standalone: true,
  imports: [CommonModule, FormsModule, LucideAngularModule],
  templateUrl: './eventos.html',
  styleUrl: './eventos.css'
})
export class EventosEncargado implements OnInit {
  readonly Plus = Plus;
  ...
  seleccionarDia(cell: CalCell): void {
    if (cell.isEmpty) return;
    this.diaSeleccionado = cell.day;

    const meses = ['Enero','Febrero','Marzo','Abril','Mayo','Junio','Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'];
    const diasSemana = ['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'];
    this.mesNombreModal = meses[this.currentMonth];

    const fecha = new Date(this.currentYear, this.currentMonth, cell.day);
    this.diaNombre = diasSemana[fecha.getDay()];

    this.eventosDelDia = this.eventos.filter(ev =>
      ev.year === this.currentYear &&
      ev.month === this.currentMonth &&
      ev.day === cell.day
    );
  ...
  esDiaSeleccionado(day: number): boolean {
    return this.mostrarModalDia && this.diaSeleccionado === day;
  }
  ...
  private construirCalendario(): void {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      cells.push({
        isEmpty: false,
        isToday: hoy.getFullYear() === this.currentYear && hoy.getMonth() === this.currentMonth && hoy.getDate() === day,
        day,
  ...
  private refrescarVista(): void {
    ...
    setTimeout(() => {
      ...
      catch (e) { console.warn('Error al refrescar vista:', e); }
    }, 0);
  }
}
