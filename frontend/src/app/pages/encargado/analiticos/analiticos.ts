...
@Component({
  selector: 'app-analiticos-encargado',
  standalone: true,
  imports: [CommonModule, LucideAngularModule],
  templateUrl: './analiticos.html',
  styleUrl: './analiticos.css'
})
export class AnaliticosEncargado implements OnInit {
  ...
  cargarAnaliticos(): void {
    ...
    forkJoin({
      ...
      error: (err: unknown) => {
        console.error('Error cargando analíticos encargado:', err);
  ...
  barHeight(bar: BarraItem): string {
    ...
    if (max === 0) return '6%';
    if (max === 1) return bar.value > 0 ? '72%' : '6%';
    if (bar.value === 0) return '6%';
  ...
  private refrescarVista(): void {
    ...
    setTimeout(() => {
      try {
        ...
        console.warn('No se pudo refrescar Analíticos encargado:', error);
      }
    }, 0);
  }
}
